...
def winklerTrunc(n, maxk, true_root, true_pk_coeff, trunc):
    # n = population
    # maxk = max degree
    ...
    for p in pks:
        ...
        c += 1    
    #takes out the zeros for the Winkler computation

    if pks.size == 0:
        winklerMeasure = 0
        sizeOfPoly = len(pks)
    elif pks.size == 1 and pks[0] == 0:
        winklerMeasure = 0
        sizeOfPoly = len(pks)
    else:
        ...
        conversion = convertFactor(n, pks)
...
def  winkler(n, maxk, true_root, true_pk_coeff):
    # n = population
    # maxk = max degree
    # true_root = the root from the PGF
    # prime_coeff = coefficients of the derivative function
...
def runFSE(excessDegree, numTrials, exp, maxk):
    ...
    for N in range(1, exp+1):    
        ...
        for deg in excessDegree:
            ...
            winklerMeasureInfinite[count] = winkler(n, maxk, true_root, true_prime_infinite_G1)
            ...
            for cutoff in [0, 0.001, 0.01, 0.1]:
                for t in range(0, numTrials):
                    ...
                    winklerMeasure[count, t]=  winkler(n, maxk, true_root, p_k_sim_G1)
                    
                    
                    
                    winkler100s[m,t] =  winklerTrunc(n, maxk, true_root, p_k_sim_G1, cutoff)
                        
                    winklerInf100s[m] = winklerTrunc(n, maxk, true_root, true_prime_infinite_G1, cutoff)
                m+=1
...
def plotting_S_and_k():
    ...
    for N in range(1, exp+1):    
        ...
        for deg in avgK:
            ...
            cut = 0.1
            thresh = 1 - stats.binom.cdf(cut,n,prob)
            thresh = 1/n
            
            # if (deg >= 2) and (n > 10):
            #     print("Stop and check")
            
            winklerMeasureInfinite[count], polyLengths[leng] = winklerTest(n, maxk, true_root, true_prime_infinite_G1, thresh)
            leng+=1
            ...
            for t in range(0, numTrials):
                ...
                winklerMeasure[count, t], polyLengths[leng] =  winklerTest(n, maxk, true_root, p_k_sim_G1, thresh)
                leng+=1
                #(1/np.abs(prime(true_root))) * np.abs(np.linalg.norm([phi(true_root)], 2))/(true_root * np.linalg.norm([denomP], 1))* np.sqrt(2/(math.pi*n)*(denomP))
        ...
        averagePoly = np.sum(polyLengths)/len(polyLengths)
        minimumSize = np.min(polyLengths)
        maxSize = np.max(polyLengths)
        
        print("The average length of a polynomial for population %d is %.3f" %(n, averagePoly))
        print("The minimum size of polynomial used in this pop is %d" %minimumSize)
        print("The maximum size of polynomial used in this pop is %d" %maxSize)
